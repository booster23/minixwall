...
struct ipt_ip {
	...
	char iniface[IF_NAMESIZE], outiface[IF_NAMESIZE];
	unsigned char iniface_mask[IF_NAMESIZE], outiface_mask[IF_NAMESIZE];
...
struct ipt_counters
{
	unsigned long pcnt, bcnt;	/* Packet and byte counters */
};
...
struct ipt_entry
{
	...
	/* jump chain (address of target chain to jump to, NULL if
		       target is a module) */
...
/* A chain is a container for a specified ruleset,
   which has a meaningful name and a list of rules */
struct ipt_chain
{
	struct list_head list;
	char name[IPT_CHAIN_MAXNAMELEN];
...
#define NF_DROP 0               /* drop packet, stop processing chain */
#define NF_ACCEPT 1             /* give unmodified packet back, stop  */
#define NF_STOLEN 2
#define NF_QUEUE 3
#define NF_REPEAT 4
...
struct ipt_tcp
{
	u16_t spts[2];			/* Source port range. */
	u16_t dpts[2];			/* Destination port range. */
	u8_t option;			/* TCP Option iff non-zero*/
	u8_t flg_mask;			/* TCP flags mask byte */
	u8_t flg_cmp;			/* TCP flags compare byte */
	u8_t invflags;			/* Inverse flags */
};
...
struct ipt_icmp
{
	u8_t type;				/* type to match */
	u8_t code[2];			/* range of code */
	u8_t invflags;			/* Inverse flags */
};
...
extern int IPT_MATCH_ITERATE(void *e, void *fn, ...);

/* fn returns 0 to continue iteration */
extern int IPT_ENTRY_ITERATE(void *entries, int size, void *fn, ...);
